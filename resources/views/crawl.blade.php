...
@php
    $defaultBreadcrumbs = [
        ...
        'Crawler' => backpack_url('plugin/movie-crawler'),
    ];
    ...
    @push('after_scripts')
        ...
        <script>
            $(document).ready(function() {
                // Tiền tố riêng cho trang ophim. Trước đây hai trang crawler dùng chung
    ...
    <script>
        ...
        $('.btn-load').click(function(e) {
            ...
            const fetchApi = async (link, from, to) => {
                isFetching = true;
                const response = await fetch("{{ backpack_url('plugin/movie-crawler/fetch') }}?" +
                    new URLSearchParams({
                        link,
                        from,
                        to
        ...
        const processMovie = async (slug) => {
            ...
            const response = await fetch("{{ backpack_url('plugin/movie-crawler/crawl') }}", {
                method: 'POST',
                headers: {
